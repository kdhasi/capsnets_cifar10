...
model.summary()
model.load_weights(filepath='weights/rgb_dynamicrouting_model.h5')

train_data, val_data, train_labels, val_labels = train_test_split(x_train, y_train, test_size=0.98, random_state=42,
                                                                  shuffle=True, stratify=y_train)

np.random.seed(42)
# Get the indices that would sort based on the class labels
sorting_indices = np.lexsort(train_labels.T)

# Use the indices to sort the data and labels
sorted_training_data = train_data[sorting_indices]
sorted_training_labels = train_labels[sorting_indices]

get_digit_capsule_output = tf.keras.backend.function([model.layers[0].input], [model.get_layer('digitcaps').output])
digit_capsule_output = get_digit_capsule_output([sorted_training_data])[0]
length_digitcaps = np.sqrt(np.sum(np.square(digit_capsule_output), axis=-1))

max_digit_capsule_index = np.argmax(length_digitcaps, axis=1)
predicted_digit_capsules = np.zeros(shape=(1000, 16))

for i in range(len(max_digit_capsule_index)):
    predicted_digit_capsules[i] = digit_capsule_output[i, max_digit_capsule_index[i]]

# flatten_digit_capsules = digit_capsule_output.reshape((digit_capsule_output.shape[0], -1))
flatten_digit_capsules = predicted_digit_capsules

tsne = TSNE(n_components=2, random_state=42)
tsne_result = tsne.fit_transform(flatten_digit_capsules)

# plt.figure(figsize=(8, 8))
# scatter = plt.scatter(tsne_result[:, 0], tsne_result[:, 1], c=np.argmax(sorted_training_labels, axis=1), cmap='tab10',
#                       alpha=0.7)
# plt.title('t-SNE Visualization of Digit Capsule Layer - Dynamic Routing (Activation Capsules Only)')
...
# Plot t-SNE points
plt.figure(figsize=(8, 8))
scatter = plt.scatter(tsne_result[:, 0], tsne_result[:, 1], c=np.argmax(y_test, axis=1), cmap='tab10', alpha=0.7)
plt.title('t-SNE Visualization of Digit Capsule Layer - Dynamic Routing (Activation Capsules Only)')

# Plot cluster centers
for class_label in unique_classes:
    class_indices = np.where(np.argmax(y_test, axis=1) == class_label)[0]
    cluster_center = np.mean(tsne_result[class_indices], axis=0)
    cluster_color = scatter.to_rgba(class_label)

    # Plot 'X' marker with the same class color
    plt.scatter(cluster_center[0], cluster_center[1], marker='X', s=100, c=cluster_color,
                label=f'Cluster {class_label} Center')
